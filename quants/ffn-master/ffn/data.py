...
def get(tickers, provider=None, common_dates=True, forward_fill=False,
    ...
    for ticker in tickers:
        ...
        if hasattr(provider, 'mcache'):
            data_[ticker] = provider(ticker=t, field=f,
                                    mrefresh=mrefresh, **kwargs)
        else:
            data_[ticker] = provider(ticker=t, field=f, **kwargs)
...
def _download_web(name, **kwargs):
    ...
    """
    return data.DataReader(name, **kwargs)
...
def yf(ticker, field, start=None, end=None, mrefresh=False):
    ...
    tmp = data.get_data_yahoo(ticker, start=start, end=end)
